...
class simplexSolver:
    ...
    def isFeasible(self):
        ...
        for i in range(0,self.data.rows):
            ...
            if ((self.w[i] > self.data.upperb[i]) or (sum < self.data.lowerb[i])):
                if printDetails: print 'infeasible... initializing with e variables'
                return False
    ...
    def createEvariables(self):
        ...
        for i in range(0,self.data.rows):
           if (self.w[i] > self.data.upperb[i]):
               ...
               if printDetails: print 'row', i, self.w[i], 'too high', self.data.upperb[i]
               self.e.append(Decimal('-1'))
               #self.data.rowVals[i].append(Decimal('-1'))
           elif (self.w[i] < self.data.lowerb[i]):
               # create addition e
               if printDetails: print 'row', i, self.w[i], 'too low', self.data.lowerb[i]
               self.e.append(Decimal('1'))
               #self.data.rowVals[i].append(Decimal('1'))
           else:
    ...
    def determineEnteringVar(self):
        ...
        for i in range(0,self.data.cols):
            if (self.resting[i] == 'lower' and self.data.objCo[i] >= 0):
                if printDetails: print(min(self.data.basic[i])), 'is eligible to enter.'
                return i
    ...
    def determineExitingVar(self,enter):
        ...
        if printDetails: print(min(self.data.nonbasic[exit['value']])), 'is eligible to exit.'
        return exit
...
# Original input from Sriram's programming assignment - TO COMPLETE
#   inputOriginal.txt
# Input that is already in the final format - COMPLETED
#   inputFinal.txt
# Input that doesn't require initialization (lecture 10, slides 10+) - COMPLETED
#   inputNoInit.txt
# Input that only requires one pivot and finishes - NEED this case???
#   inputOnePivot.txt
# Input that cycles (requires blands rule) - TO CREATE
#   inputCycling.txt
# Input that demonstrates degeneracy - TO CREATE
#   inputDegenerate.txt
